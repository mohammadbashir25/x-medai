    ...
    >
      ...
      >
        {HERO_HEADING_LINES.map((line, i) => (
          <span key={line} className={` ${i < 1 ? "block" : "block text-primary"}`}>
            {line}
          </span>
        ))}
      </motion.h1>
      ...
      <motion.div variants={itemVariants} className="mt-10 flex items-center gap-4">
        <Link
          href="/contact"
          className="group relative rounded-full bg-primary px-7 py-3 text-sm font-medium text-primary-foreground transition-transform duration-300 hover:-translate-y-0.5"
        >
